...
import { slimAlgoliaRecord } from './src/lib/algolia/slim-for-algolia'
 ...
 */
function isValidSendGridApiKey(apiKey: string): boolean {
 ...
 */
function validateSendGridConfig(): { isValid: boolean; errors: string[] } {
  const errors: string[] = []
...
// databaseDriverOptions is what Medusa uses for the DB connection (not databaseExtra).
// When connecting to localhost/127.0.0.1 (e.g. Cloud SQL Proxy), do not use SSL and allow longer connect timeout.
...
if (isLocalDb && dbUrl) {
  ...
  const add = params.filter((p) => !existing.includes(p.split('=')[0] + '=')).join('&')
  if (add) dbUrl = dbUrl + sep + add
}
let databaseDriverOptions: Record<string, unknown> = {}
if (isLocalDb) {
  ...
  if (process.env.DATABASE_EXTRA) {
    try {
      const extra = JSON.parse(process.env.DATABASE_EXTRA) as Record<string, unknown>
...
const config: any = {
  projectConfig: {
    ...
    http: {
      ...
      // Local-only auth: email/password only (no GitHub or other OAuth)
      authMethodsPerActor: {
        user: ["emailpass"],
        customer: ["emailpass"],
      },
                  ...
                  transformer: (product: any) => {
                    const doc: Record<string, unknown> = {
                      ...
                      variant_sku: product.variants
                        ...
                        .filter(Boolean)
                        .join(" "),
                      options: product.options?.map((o: any) => o.title).join(" "),
...
// Fulfillment module with dynamic delivery provider (5% fee if order >= $50, else 10%)
config.modules.fulfillment = {
  resolve: "@medusajs/medusa/fulfillment",
  options: {
    providers: [
      { resolve: "@medusajs/fulfillment-manual", id: "manual" },
      { resolve: "./src/fulfillment/dynamic-delivery-provider", id: "dynamic_delivery" },
    ],
  },
}
