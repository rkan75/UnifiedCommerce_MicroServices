/**
 * Example: scheduled subscription processing (Medusa 2 app).
 *
 * This file is NOT auto-loaded. Use it as a template inside your store backend:
 *
 * Option A — `medusa exec` from cron:
 *   npx medusa exec ./src/jobs/process-subscriptions.ts
 *
 * Option B — register a scheduled job in your module (see Medusa 2 jobs API).
 *
 ...
 * 5. On success → create order from subscription lines (workflow / API).
