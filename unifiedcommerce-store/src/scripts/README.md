...
## Import Amazon GNC products from `amazon_gnc_product_details.xlsx`

Upserts products from `data/amazon_gnc_product_details.xlsx` (Amazon scrape: titles, images, price, ingredients, etc.). New rows are **created**; existing rows are matched by handle `amazon-{asin}` and **updated**.

- Sets **product type** to your Health & Wellness type (default id `ptyp_01KM44HB9H03N9JPVC3Q79Y4XE`, override with `PRODUCT_TYPE_ID`).
- Writes **metadata** from every Excel column (prefixed `excel_*`) plus `import_source`, `amazon_asin`, `amazon_product_url`, and stock fields.
- Sets **variant price** (first region’s currency), **images** / thumbnail, **variant–image** links.
- Sets **inventory** at the default stock location (`DEFAULT_STOCKED_QUANTITY`, default `100`; override location with `STOCK_LOCATION_ID`).

**From `unifiedcommerce-store/` (database reachable):**

```bash
npm run import-amazon-gnc-products
# or
npx medusa exec ./src/scripts/import-amazon-gnc-products-from-xlsx.ts
```

**Optional env:** `EXCEL_FILE`, `PRODUCT_TYPE_ID`, `DEFAULT_STOCKED_QUANTITY`, `STOCK_LOCATION_ID`, `SALES_CHANNEL_ID`, `SHIPPING_PROFILE_ID`, `IMPORT_BATCH_SIZE`.

Requires a **sales channel**, **shipping profile**, and **region** in the DB (normal Medusa seed). If inventory linking fails, set `STOCK_LOCATION_ID` to your warehouse location id from Admin → Settings → Locations.
...
## Set all products to one product type (e.g. grocery)

Assigns every product the same **product type** (creates the type if missing). Default value is **`grocery`** (shown in admin as the type label).

```bash
npx medusa exec ./src/scripts/set-all-products-product-type.ts
```

Optional: `PRODUCT_TYPE_VALUE=Organic` to use a different label (must match what you want in admin).

The script sets the product **`type_id`** foreign key (not a nested `type` object), so bulk updates do not hit duplicate product-type errors from the ORM.
...
```bash
npx medusa exec ./src/scripts/reset-admin-password.ts -- admin@example.com YourNewPassword123
```
