...
/**
 * Sets the cart's shipping address and selected delivery store (e.g. for "Shop for Delivery").
...
export async function setDeliveryAddressAndStore(
  ...
  try {
    ...
    const metadata: Record<string, unknown> = {
      ...
      delivery_store_name: store.name,
      delivery_store_address: `${store.address}, ${store.city}, ${store.state} ${store.zip}`,
    }
