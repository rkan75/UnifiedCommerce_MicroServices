...
import { findCategoryInTree } from "@lib/util/plp-department-categories"
...
const StoreTemplate = async ({
  ...
  const { collections: plpCollections = [] } = await listCollections({}, countryCode).catch(
    () => ({ collections: [] as HttpTypes.StoreCollection[] })
  )

  const categoryName = categoryId
    ? findCategoryInTree(categories || [], categoryId)?.name
    : undefined

  const collectionName = collectionId
    ? plpCollections.find((c) => c.id === collectionId)?.title
    : undefined

  const collectionsSorted = [...plpCollections].sort((a, b) =>
