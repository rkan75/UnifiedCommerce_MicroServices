    ...
    if (!variantId?.trim()) {
      toast.error(
        listLabels.isShopInStoreList
          ? "Cannot add to list: missing product variant."
          : "Cannot add to cart: missing product variant."
      )
      return
    }
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : listLabels.isShopInStoreList
            ? "Could not add ingredients to list"
            : "Could not add ingredients to cart"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <section className="lg:col-span-1" aria-labelledby="ingredients-heading">
          ...
          <div className="mt-6">
            <Button
              variant="primary"
              className={clx("w-full gap-1.5", PRIMARY_ADD_TO_CART_BUTTON_CLASS)}
