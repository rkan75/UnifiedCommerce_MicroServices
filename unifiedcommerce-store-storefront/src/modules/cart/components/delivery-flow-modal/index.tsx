...
import { writeHeaderFulfillmentPreference } from "@lib/config/checkout-method-options"

/** Matches Tailwind `header.red` (GNC accent). */
...
type DeliveryFlowModalProps = {
  ...
  initialStep?: 1 | 2 | 3 | 4
  /** After success, refresh cart but do not redirect to checkout (e.g. header delivery dropdown). */
  stayOnStorefrontAfterSuccess?: boolean
}
...
export default function DeliveryFlowModal({
  ...
  stayOnStorefrontAfterSuccess = false,
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const lookup = getDeliveryFulfillmentStoreLookupEnabled()
      const rawStart = initialStepProp ?? 1
      const startStep = !lookup && rawStart === 2 ? 1 : rawStart
      setError(null)
      ...
      if (lookup && (startStep === 2 || startStep === 3)) {
        ...
        if (
          startStep === 3 &&
          meta?.delivery_slot_date_iso &&
          meta?.delivery_slot_start &&
          meta?.delivery_slot_end
        ) {
      ...
      } else if (!lookup && startStep === 3 && meta?.delivery_slot_date_iso && meta?.delivery_slot_start && meta?.delivery_slot_end) {
        const dateIso = meta.delivery_slot_date_iso
        const slotDate = new Date(dateIso + "T12:00:00")
        const dayOpt = getDeliveryDayOptions().find((d) => d.dateKey === dateIso)
        if (dayOpt) setSelectedDay(dayOpt)
    ...
    try {
      ...
      const stores =
        apiStores.length > 0
          ...
          : getNearestStores(lat, lng, topStoresLimit)
      setTopStores(stores)
  ...
  }, [getAddressForGeocode, selectedAddress, formData, topStoresLimit])

  const handleNextFromStep1 = useCallback(async () => {
    if (!deliveryFulfillmentLookup) {
      const addr = getAddressForGeocode()
  ...
  const handleContinueToPayAndPlaceOrder = useCallback(async () => {
    if (deliveryFulfillmentLookup && !selectedStore) {
    ...
    try {
      ...
      const result = await setDeliveryAddressAndStore(
        address,
        storePayload,
        email,
        {
      ...
      if (result.success) {
        ...
        onSuccess?.()
        close()
        if (!stayOnStorefrontAfterSuccess) {
          // List-mode “Order online” keeps uc_header_fulfillment=in_store until now; checkout would redirect to /cart.
          writeHeaderFulfillmentPreference("delivery")
          router.push(
            `/${countryCode}/checkout?step=delivery&shipping=standard`
          )
        }
        router.refresh()
      } else {
    ...
    >
      ...
      <Modal.Body>
        <div className="flex flex-col gap-4 py-2">
          ...
          {step === 1 && (
            <>
              <Text className="text-small-regular" style={{ color: HEADER_RED }}>
                {deliveryFulfillmentLookup ? (
                  <>
                  ...
                  <>
                    Select a saved address or enter a new delivery address. Next,
              ...
              {hasSavedAddresses && (
                <div className="flex flex-col gap-2">
                  <Text className="text-small-medium" style={{ color: HEADER_RED }}>
                    Saved addresses
                  </Text>
                  <AddressSelect
          ...
          {step === 2 && (
            <>
              <Text className="text-small-regular text-header-red">
                Nearest top {topStoresLimit} stores within {STORE_RADIUS_MILES}{" "}
                miles of your delivery address (by zip). Select one to continue.
              </Text>
              {topStores.length === 0 ? (
          ...
          {step === 4 && (
            <>
              <Text className="text-small-regular mb-2 text-header-red">
        ...
        <div className="w-full flex justify-center items-center gap-2">
          ...
          {step === 3 && (
            <>
              <Button
                variant="secondary"
                onClick={() => setStep(deliveryFulfillmentLookup ? 2 : 1)}
              >
                Back
              </Button>
              <Button
