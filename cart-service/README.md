...
When the storefront sets **CART_SERVICE_URL** (e.g. `http://localhost:8083`), all `sdk.store.cart.*` usage is replaced with calls to this service. Shipping options (GET /store/shipping-options) remain from the Medusa backend.

## Implemented API (replaces sdk.store.cart.*)

| Method | Path | Purpose |
|--------|------|---------|
| GET | `/store/carts/:id` | Get cart (optional `fields` ignored). |
...
- **Same PostgreSQL** as Medusa (e.g. `grocery_store`). Run **`src/main/resources/schema.sql`** once to create:
  - `commerce_cart`
  - `commerce_line_item`
  - `commerce_order`
...
| `SPRING_DATASOURCE_URL` | JDBC URL (same DB as Medusa). For "cannot execute INSERT in a read-only transaction" (25006), ensure you connect to a read-write instance; the service also sets `default_transaction_read_only = off` via Hikari connection-init-sql. | `jdbc:postgresql://127.0.0.1:5433/grocery_store` |
...
1. **Tables:** Create cart tables once (use the **same** database the service will use):
   ...
   ```
   The script uses `SPRING_DATASOURCE_*` from the environment, or defaults to `127.0.0.1:5433/grocery_store`. It also loads `../.env` if present so it matches `start-all.sh`.
...
1. From **repo root**, ensure a `.env` exists with the DB you want (e.g. `SPRING_DATASOURCE_URL=jdbc:postgresql://127.0.0.1:5433/grocery_store` and same host/port/db for all services).
...
1. **Database and role:** Run as superuser so `grocery_store` and role `grocery_app` are not forced read-only:
   ```bash
...
```bash
psql -h 127.0.0.1 -p 5433 -U postgres -d grocery_store -f ensure-cart-app-user-access.sql
```
